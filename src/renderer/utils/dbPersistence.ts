...
export async function initializePouchDbPersistence(): Promise<void> {
  ...
  chatState$.onChange((value) => {
    try {
      const transformedValue = chatStateTransform(value);
      queueSaveOperation('chat-state', transformedValue);
    } catch (error) {
      console.error('Error transforming chat state for PouchDB:', error);
    }
  });
  ...
  setupStatePersistence(mcpServersState$, 'mcp-servers-state');
...
async function preloadAllRevisions(): Promise<void> {
  try {
    ...
    for (const key of keys) {
      ...
      try {
        const doc = await trpcProxyClient.storage.getData.query({ id: docId });
        if (doc && '_rev' in doc) {
...
async function saveStateToPouchDb(key: string, value: any): Promise<void> {
  ...
  try {
    ...
    try {
      const currentDoc = await trpcProxyClient.storage.getData.query({
        id: docId,
      });
    ...
    if (currentRevision) {
      ...
      const result = await trpcProxyClient.storage.saveData.mutate({
        id: docId,
        data: {
    ...
    if (error.message && error.message.includes('conflict')) {
      ...
      try {
        ...
        const latestDoc = await trpcProxyClient.storage.getData.query({
          id: docId,
        });
...
async function loadStateFromPouchDb(): Promise<void> {
  try {
    ...
    // Load chats state
    await loadStateData(chatState$, 'chat-state');
    ...
    // Load mcp servers state
    await loadStateData(mcpServersState$, 'mcp-servers-state');
...
async function loadStateData(
  ...
  try {
    ...
    const data = await trpcProxyClient.storage.getData.query({ id: docId });
  ...
  return {
    ...
    custom: {
      ...
      load: async () => {
        try {
          const docId = `legend_state_${key}`;
          const result = await trpcProxyClient.storage.getData.query({
            id: docId,
          });
