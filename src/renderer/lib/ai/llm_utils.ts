export function normalizeOpenAIApiHostAndPath(options: {
  apiHost?: string;
  apiPath?: string;
}) {
  ...
  if (
    apiHost.endsWith('://api.openai.com') ||
    apiHost.endsWith('://api.openai.com/v1')
  ) {
    apiHost = DEFAULT_HOST;
    apiPath = DEFAULT_PATH;
    return { apiHost, apiPath };
  ...
  if (
    apiHost.endsWith('://openrouter.ai') ||
    apiHost.endsWith('://openrouter.ai/api')
  ) {
